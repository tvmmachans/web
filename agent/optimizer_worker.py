...
class OptimizerWorker:
    ...
    async def daily_trend_update(self):
        ...
        try:
            ...
            logger.info(
                f"Daily trend update completed. Generated {len(content_suggestions)} content suggestions"
            )
    ...
    async def daily_performance_analysis(self):
        ...
        try:
            ...
            logger.info(
                f"Daily performance analysis completed. Generated {len(insights)} insights"
            )

        except Exception as e:
            logger.error(f"Daily performance analysis failed: {e}")

    async def optimize_specific_post(
        self, post_id: int, platform: str
    ) -> Dict[str, Any]:
        ...
        try:
            async with async_session() as session:
                ...
                return {
                    ...
                    "top_trends": trends[:5],
                    "trend_velocity_avg": (
                        sum(t.velocity for t in trends) / len(trends) if trends else 0
                    ),
                }
    ...
    async def _generate_content_suggestions(
        self, trend_insights: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for trend in malayalam_trends[:3]:  # Top 3 trends
                suggestion = {
                    ...
                    "optimal_posting_time": await self._get_optimal_time_for_platform(
                        trend.platform
                    ),
            ...
            for platform, trends in platform_trends.items():
                if len(trends) > 1:
                    suggestion = {
                        ...
                        "engagement_potential": sum(
                            t.trend_strength for t in trends[:2]
                        )
                        / 2,
                    }
                    suggestions.append(suggestion)
    ...
    async def _validate_model_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_data = await self.learning_manager.ml_service.collect_training_data(
                limit=100
            )
            ...
            mae = sum(abs(a - p) for a, p in zip(actual_views, predicted_views)) / len(
                actual_views
            )
        ...
        try:
            if validation_results.get("status") == "success":
                ...
                if accuracy < 0.7:  # 70% threshold
                    logger.warning(
                        f"Model accuracy {accuracy:.2f} below threshold, retraining..."
                    )
                    await self.learning_manager.retrain_models()
    ...
    async def _generate_performance_insights(
        ...
        try:
            ...
            # Platform insights
            best_platform = (
                max(platform_performance.items(), key=lambda x: x[1]["views"])
            ...
            if total_posts > 10:
                insights.append(
                    "High posting frequency - consider quality over quantity"
                )
    ...
    async def _update_learning_from_performance(
        self, performance_data: Dict[str, Any], insights: List[str]
    ):
    ...
    async def _generate_post_optimizations(
        self, post_data: Dict[str, Any], trends: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        try:
            ...
            if len(title) < 30:
                optimizations.append(
                    "Title is too short - consider making it more descriptive"
                )
            ...
            if trends:
                ...
                if top_trend.lower() not in caption.lower():
                    optimizations.append(
                        f"Consider incorporating trending topic: {top_trend}"
                    )
            ...
            if scheduled_time:
                ...
                if not (18 <= hour <= 21):  # Peak Malayalam viewing hours
                    optimizations.append(
                        "Consider posting during peak Malayalam viewing hours (6-9 PM)"
                    )
    ...
    async def _get_optimal_time_for_platform(self, platform: str) -> str:
        ...
        try:
            ...
            if platform_time:
                ...
                return (
                    "Tuesday at 19:00" if platform == "youtube" else "Friday at 18:00"
                )
