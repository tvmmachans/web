...
from database import get_db, Trends, LearningData, ModelMetrics, PostingOptimization
...
async def get_analytics(
    ...
    try:
        ...
        query = query.filter(Post.posted_at >= start_date).group_by(Post.platform)
        ...
        for row in analytics_data:
            response.append(AnalyticsResponse(
...
async def get_trends(
    ...
    """
    try:
        query = db.query(Trends).order_by(desc(Trends.trend_strength))

        if platform:
            query = query.filter(Trends.platform == platform)

        if language:
            query = query.filter(Trends.language == language)
...
@router.get("/optimization-suggestions", response_model=List[OptimizationSuggestionsResponse])
async def get_optimization_suggestions(
    ...
    """
    try:
        query = db.query(PostingOptimization).order_by(desc(PostingOptimization.engagement_score))

        if platform:
            query = query.filter(PostingOptimization.platform == platform)

        if language:
            query = query.filter(PostingOptimization.language == language)
        ...
        for opt in optimizations:
            ...
            day_name = day_names[opt.optimal_day] if 0 <= opt.optimal_day < 7 else "Monday"

            recommendation = f"Post on {opt.platform} at {opt.optimal_hour}:00 on {day_name} (Expected engagement: {opt.engagement_score:.3f})"

            response.append(OptimizationSuggestionsResponse(
...
async def get_learning_insights(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        model_metrics_result = await db.execute(
            db.query(ModelMetrics)
            .filter(ModelMetrics.model_name == "engagement_prediction")
            .order_by(desc(ModelMetrics.training_date))
            .limit(1)
        )
        latest_metrics = model_metrics_result.scalar()
        ...
        trends_result = await db.execute(
            db.query(Trends.topic)
            .filter(Trends.language == "ml")
        ...
        learning_progress = {
            "data_collection": min(learning_count / 1000, 1.0),  # Progress towards 1000 samples
            "model_accuracy": model_accuracy or 0.0,
            "trend_coverage": min(len(recent_trends) / 10, 1.0)  # Progress towards 10 trends
        }
