...
class OrchestratorEventIntegration:
    ...
    async def _handle_pipeline_completed(self, event_data: Dict[str, Any]):
        ...
        await self.publish_event('backend.pipeline_completed', {
            'task_id': task_id,
    ...
    async def _handle_pipeline_failed(self, event_data: Dict[str, Any]):
        ...
        await self.publish_event('backend.pipeline_failed', {
            'task_id': task_id,
            'error': error,
    ...
    async def _handle_posted(self, event_data: Dict[str, Any]):
        ...
        await self.publish_event('backend.post_published', {
            'task_id': task_id,
            'post_id': post_id,
            'platforms': platforms
        })
    ...
    async def _store_caption_analytics(self, task_id: str, caption: str, hashtags: list):
    ...
    async def _store_performance_analytics(self, task_id: str, analytics: Dict[str, Any]):
    ...
    async def publish_video_upload(self, video_id: str, user_id: str, metadata: Dict[str, Any]):
        """Publish video upload event."""
        await self.publish_event('backend.video_uploaded', {
            'video_id': video_id,
            'user_id': user_id,
            'metadata': metadata
        })

    async def publish_caption_generated(self, video_id: str, caption: str, hashtags: list):
        """Publish caption generation event."""
        await self.publish_event('backend.caption_generated', {
            'video_id': video_id,
            'caption': caption,
            'hashtags': hashtags
        })

    async def publish_post_scheduled(self, post_id: str, scheduled_time: str, platforms: list):
        """Publish post scheduling event."""
        await self.publish_event('backend.post_scheduled', {
            'post_id': post_id,
            'scheduled_time': scheduled_time,
            'platforms': platforms
        })

    async def publish_analytics_collected(self, post_id: str, analytics: Dict[str, Any]):
        """Publish analytics collection event."""
        await self.publish_event('backend.analytics_collected', {
            'post_id': post_id,
            'analytics': analytics
        })
