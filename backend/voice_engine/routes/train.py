...
from models.voice_models import VoiceProfile, TrainingJob, AudioSample
...
async def upload_audio_samples(
    ...
    try:
        ...
        if len(files) != len(transcripts):
            raise HTTPException(status_code=400, detail="Number of files must match number of transcripts")
...
async def start_voice_training(
    ...
    try:
        ...
        if sample_count < 10:
            raise HTTPException(status_code=400, detail="At least 10 audio samples required for training")
